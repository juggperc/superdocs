...
const assetMeta: Record<AssetType, { icon: typeof FileText; color: string; glowClass: string; bgClass: string }> = {
  doc: { icon: FileText, color: "text-blue-400", glowClass: "group-hover:shadow-[0_0_25px_oklch(0.65_0.15_250/10%)]", bgClass: "bg-blue-400/10" },
  sheet: { icon: Grid, color: "text-emerald-400", glowClass: "group-hover:shadow-[0_0_25px_oklch(0.70_0.15_160/10%)]", bgClass: "bg-emerald-400/10" },
  slide: { icon: Presentation, color: "text-amber-400", glowClass: "group-hover:shadow-[0_0_25px_oklch(0.70_0.15_60/10%)]", bgClass: "bg-amber-400/10" },
  warp: { icon: Sparkles, color: "text-primary", glowClass: "group-hover:shadow-[0_0_25px_oklch(0.65_0.18_260/10%)]", bgClass: "bg-primary/10" },
}
  ...
  useEffect(() => {
    ...
    if (savedAssets) {
      ...
      const initialDoc: Asset = {
        ...
        content: `<h2>Welcome to Warp Suite</h2><p>This is your first document. Open the Warp Assistant on the right to try AI-powered editing.</p><h3>Quick Start</h3><p>1. Click the <strong>+</strong> button to create docs, sheets, or slides.</p><p>2. Open <strong>Settings</strong> to add your OpenRouter API key.</p><p>3. Use the <strong>Warp Assistant</strong> to edit with AI across your workspace.</p>`,
      ...
      {currentView === "library" ? (
        <div className="flex-1 flex overflow-hidden">
          <div className="w-56 border-r border-white/[0.06] bg-[oklch(0.12_0.005_260)] flex flex-col hidden md:flex">
            <div className="p-4 flex items-center gap-2 mb-1">
              ...
              <span className="font-semibold text-sm tracking-tight">Warp Suite</span>
            </div>
            ...
            <div className="px-3 pb-2 flex items-center justify-between group">
              ...
              <button
                ...
                className="opacity-0 group-hover:opacity-100 p-1 text-muted-foreground hover:text-foreground transition-all rounded hover:bg-white/[0.06]"
              >
            ...
            <div className="flex-1 overflow-y-auto px-2 space-y-0.5">
              {workspaces.map((ws) => (
                <button
                  ...
                  className={`w-full text-left px-2.5 py-1.5 rounded-lg text-sm transition-all flex items-center justify-between group ${
              ...
              <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-10">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {workspaceAssets.length} {workspaceAssets.length === 1 ? "file" : "files"}
                  </p>
                </div>

                <div className="flex items-center gap-1 glass rounded-lg p-1">
                  <Button variant="ghost" size="sm" onClick={() => handleNewAsset("warp")} className="text-primary hover:text-primary hover:bg-primary/10 font-medium h-8 px-3 gap-1.5 text-xs rounded-md">
                    <Sparkles className="w-3.5 h-3.5" /> Warp
                  </Button>
                  <div className="w-px h-4 bg-white/[0.08]" />
                  <Button variant="ghost" size="sm" onClick={() => handleNewAsset("doc")} className="text-muted-foreground hover:text-foreground h-8 px-3 gap-1.5 text-xs rounded-md">
                    <FileText className="w-3.5 h-3.5" /> Doc
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => handleNewAsset("sheet")} className="text-muted-foreground hover:text-foreground h-8 px-3 gap-1.5 text-xs rounded-md">
                    <Grid className="w-3.5 h-3.5" /> Sheet
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => handleNewAsset("slide")} className="text-muted-foreground hover:text-foreground h-8 px-3 gap-1.5 text-xs rounded-md">
              ...
              {workspaceAssets.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-28 glass rounded-2xl">
                  ...
                  <p className="text-sm text-muted-foreground mb-6 max-w-xs text-center">
                    Create a doc, sheet, slide, or start a Warp AI session.
                  </p>
                  <Button onClick={() => handleNewAsset("doc")} className="bg-primary text-primary-foreground rounded-lg px-5 h-8 text-sm glow-sm">
                    Create Doc
                  </Button>
                </div>
              ) : (
                  ...
                  {workspaceAssets.map((asset) => {
                    ...
                    return (
                      <div
                        ...
                        onKeyDown={(e) => { if (e.key === "Enter") openAsset(asset.id) }}
                        role="button"
                        tabIndex={0}
                        className={`group glass rounded-xl hover:bg-white/[0.06] transition-all duration-200 flex flex-col h-52 text-left outline-none focus-visible:ring-1 focus-visible:ring-primary overflow-hidden cursor-pointer ${meta.glowClass}`}
                      >
                        <div className="flex-1 w-full p-5 flex items-center justify-center relative">
                          <div className="absolute inset-0 bg-gradient-to-b from-white/[0.02] to-transparent pointer-events-none" />
                          <Icon className={`w-10 h-10 ${meta.color} opacity-30 group-hover:opacity-60 group-hover:scale-110 transition-all duration-300`} />
                        </div>

                        <div className="p-3 pt-0 flex flex-col gap-0.5 z-10 shrink-0 w-full border-t border-white/[0.04]">
                          <div className="flex items-center justify-between w-full">
                            <div className="flex items-center gap-2 truncate pr-2">
                              <div className={`w-5 h-5 rounded ${meta.bgClass} flex items-center justify-center shrink-0`}>
                                <Icon className={`w-2.5 h-2.5 ${meta.color}`} />
                              </div>
                              <h3 className="font-medium text-foreground text-[13px] truncate">{asset.title}</h3>
                            </div>
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-36 glass-strong bg-[oklch(0.16_0.005_260)] border-white/[0.08]">
                                <DropdownMenuItem onClick={(e) => handleRenameAsset(asset.id, asset.title, e as any)} className="text-sm">
                                  <Edit2 className="w-3.5 h-3.5 mr-2" /> Rename
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={(e) => handleDeleteAsset(asset.id, e as any)} className="text-red-400 focus:text-red-400 focus:bg-red-400/10 text-sm">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
              ...
              {openAssetIds.map((id) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`h-[30px] min-w-[120px] max-w-[200px] px-3 rounded-t-lg flex items-center justify-between gap-1.5 cursor-pointer relative group transition-all text-xs ${
                      isActive
                        ? "bg-background border-t border-x border-white/[0.08]"
                  ...
                  >
                    ...
                    <span className="flex items-center gap-1.5 flex-1 overflow-hidden">
                      ...
                      <span className={`truncate select-none ${isActive ? "font-medium text-foreground" : ""}`}>
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            {activeAsset?.type !== "warp" && (
              <AgentSidebar
                onAgentStateChange={setIsAgentActive}
                editorRef={editorRef}
        ...
        {showSettings && (
          ...
          >
            <motion.div
              ...
              className="glass-strong bg-[oklch(0.15_0.005_260)] rounded-xl w-full max-w-md p-6 shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-base font-semibold">Settings</h2>
                <button type="button" onClick={() => setShowSettings(false)} className="p-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-white/[0.06] transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="flex justify-end gap-2 mt-6">
                <Button variant="ghost" size="sm" onClick={() => setShowSettings(false)} className="h-8 px-3 text-xs">
                  Cancel
                </Button>
                <Button size="sm" onClick={saveSettings} className="bg-primary text-primary-foreground h-8 px-4 text-xs glow-sm">
                  Save
                </Button>
              </div>
            </motion.div>
